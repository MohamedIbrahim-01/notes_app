...
notesNavBtn.addEventListener("click", () =>
  app.classList.remove("switch-section")
);

addNavBtn.addEventListener("click", () => app.classList.add("switch-section"));

plusIconBtn.addEventListener("click", () =>
  app.classList.add("switch-section")
);

arrowIconBtn.addEventListener("click", () =>
  archiveNotes.classList.toggle("archive-close")
);
...
const deleteNote = (e, index) => {
  const answer = confirm("Delete Note ?");
  if (!answer) return;
  const notes = fetchData("notes");
...
viewCloseIcon.addEventListener("click", () =>
  sectionElement.classList.remove("note-checked")
);
